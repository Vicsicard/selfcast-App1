"""
Audio cutting utilities with robust error handling.
"""
...
class AudioCutter:
    """Handles cutting audio files into segments with error handling."""
    
    def __init__(self, input_path: str, output_dir: str, job_id: str):
        """Initialize audio cutter with error handling.
        ...
        self.output_dir = Path(output_dir) / job_id / "audio_chunks"
    ...
    def cut_segment(self, chunk_num: int, start_time: float, end_time: float) -> Optional[str]:
        """Cut a single audio segment using ffmpeg.
        ...
        output_path = self.output_dir / f"{chunk_id}.m4a"
    ...
    def process_chunks(self, segments: List[Union[Dict, TranscriptSegment]]) -> List[Dict]:
        ...
        Returns:
            List of processed chunks with audio paths added
        """
        processed_chunks = []
        failed_chunks = []
        ...
        for i, segment in enumerate(segments, 1):
            try:
                ...
                audio_path = self.cut_segment(i, chunk["start"], chunk["end"])
        ...
        if failed_chunks:
            self.error_handler.log_video_error(
                f"Failed to process {len(failed_chunks)} audio chunks: {failed_chunks}"
            )
